...
class OCR_DataLoader(data.Dataset):
    ...
    def _list_files(self, check_all_imgs=False):
        ...
        for dataset_idx, dataset_name in enumerate(self.cfg.DATASET.TRAIN.DATASET_NAME_LIST):
            ...
            image_path = os.path.join(self.ocr_dataset_catalog.root_path,dataset_attrs['root_path'])

            train_json_path = os.path.join(self.ocr_dataset_catalog.root_path) + '/' + dataset_attrs['gt_path']
    ...
    def _shrink(self, bboxes, rate, max_shr=20):
        ...
        for bbox in bboxes:
            ...
            # ============================= change : Waring =============================
            shrinked_bbox = np.array(np.array(shrinked_bbox,dtype=object)[0],dtype=object)
